...
real_T det(const emlrtStack *sp, const emxArray_real_T *x)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    emxInit_int32_T(&st, &ipiv, 2, &nc_emlrtRTEI, true);
    emxInit_ptrdiff_t(&st, &ipiv_t, 1, &oc_emlrtRTEI, true);
    emxInit_ptrdiff_t(&st, &r, 1, &lc_emlrtRTEI, true);
    b_st.site = &mb_emlrtRSI;
    c_st.site = &ob_emlrtRSI;
    repmat(&c_st, (ptrdiff_t)0.0, muIntScalarMin_sint32(m, n), r);
    i = ipiv_t->size[0];
    ipiv_t->size[0] = r->size[0];
    emxEnsureCapacity_ptrdiff_t(&b_st, ipiv_t, i, &lc_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&b_st, ipiv, i, &mc_emlrtRTEI);
