                       ...
                       emxArray_real_T *Sigma)
{
  ...
  i = w->size[0];
  w->size[0] = gm->size[0];
  emxEnsureCapacity_real_T(sp, w, i, &pb_emlrtRTEI);
  i = mu->size[0] * mu->size[1];
  mu->size[0] = gm->data[0].mu->size[0];
  mu->size[1] = gm->size[0];
  emxEnsureCapacity_real_T(sp, mu, i, &qb_emlrtRTEI);
  ...
  emxEnsureCapacity_real_T(sp, Sigma, i, &rb_emlrtRTEI);
  i = gm->size[0];
  for (b_i = 0; b_i < i; b_i++) {
    if (b_i + 1 > gm->size[0]) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, gm->size[0], &cb_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (b_i + 1 > w->size[0]) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, w->size[0], &db_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    w->data[b_i] = gm->data[b_i].w;
    if (b_i + 1 > mu->size[1]) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, mu->size[1], &ab_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (b_i + 1 > gm->size[0]) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, gm->size[0], &x_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    emlrtSubAssignSizeCheckR2012b(&mu->size[0], 1, &gm->data[b_i].mu->size[0],
                                  1, &o_emlrtECI, (emlrtCTX)sp);
    loop_ub = gm->data[b_i].mu->size[0];
    for (i1 = 0; i1 < loop_ub; i1++) {
      mu->data[i1 + mu->size[0] * b_i] = gm->data[b_i].mu->data[i1];
    }
    if (b_i + 1 > Sigma->size[2]) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, Sigma->size[2], &bb_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (b_i + 1 > gm->size[0]) {
