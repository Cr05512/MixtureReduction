            ...
            emxArray_int32_T *jpvt)
{
  ...
  emxEnsureCapacity_int32_T(sp, jpvt, i, &tc_emlrtRTEI);
  b_na = A->size[1];
  for (i = 0; i < b_na; i++) {
    jpvt->data[i] = 0;
  }
  ...
  i = tau->size[0];
  tau->size[0] = minmana;
  emxEnsureCapacity_real_T(&st, tau, i, &uc_emlrtRTEI);
  emxInit_ptrdiff_t(&st, &jpvt_t, 1, &vc_emlrtRTEI, true);
  if ((A->size[0] == 0) || (A->size[1] == 0) || (A->size[0] < 1) ||
    ...
    emxEnsureCapacity_ptrdiff_t(&st, jpvt_t, i, &vc_emlrtRTEI);
    b_na = A->size[1];
    for (i = 0; i < b_na; i++) {
