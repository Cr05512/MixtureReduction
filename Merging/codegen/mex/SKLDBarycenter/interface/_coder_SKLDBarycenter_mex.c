/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * _coder_SKLDBarycenter_mex.c
 *
 * Code generation for function '_coder_SKLDBarycenter_mex'
 *
 */

/* Include files */
#include "_coder_SKLDBarycenter_mex.h"
#include "SKLDBarycenter_data.h"
#include "SKLDBarycenter_initialize.h"
#include "SKLDBarycenter_terminate.h"
#include "_coder_SKLDBarycenter_api.h"
#include "rt_nonfinite.h"

/* Function Definitions */
void SKLDBarycenter_mexFunction(int32_T nlhs, mxArray *plhs[1], int32_T nrhs,
                                const mxArray *prhs[3])
{
  emlrtStack st = {
      NULL, /* site */
      NULL, /* tls */
      NULL  /* prev */
  };
  const mxArray *outputs;
  st.tls = emlrtRootTLSGlobal;
  /* Check for proper number of arguments. */
  if (nrhs != 3) {
    emlrtErrMsgIdAndTxt(&st, "EMLRT:runTime:WrongNumberOfInputs", 5, 12, 3, 4,
                        14, "SKLDBarycenter");
  }
  if (nlhs > 1) {
    emlrtErrMsgIdAndTxt(&st, "EMLRT:runTime:TooManyOutputArguments", 3, 4, 14,
                        "SKLDBarycenter");
  }
  /* Call the function. */
  SKLDBarycenter_api(prhs, &outputs);
  /* Copy over outputs to the caller. */
  emlrtReturnArrays(1, &plhs[0], &outputs);
}

void mexFunction(int32_T nlhs, mxArray *plhs[], int32_T nrhs,
                 const mxArray *prhs[])
{
  mexAtExit(&SKLDBarycenter_atexit);
  /* Module initialization. */
  SKLDBarycenter_initialize();
  /* Dispatch the entry-point. */
  SKLDBarycenter_mexFunction(nlhs, plhs, nrhs, prhs);
  /* Module termination. */
  SKLDBarycenter_terminate();
}

emlrtCTX mexFunctionCreateRootTLS(void)
{
  emlrtCreateRootTLSR2021a(&emlrtRootTLSGlobal, &emlrtContextGlobal, NULL, 1,
                           NULL);
  return emlrtRootTLSGlobal;
}

/* End of code generation (_coder_SKLDBarycenter_mex.c) */
