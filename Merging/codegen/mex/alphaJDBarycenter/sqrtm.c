             ...
             emxArray_creal_T *X)
{
  ...
  emxInit_creal_T(sp, &R, 2, &ve_emlrtRTEI, true);
  emxInit_creal_T(sp, &Q, 2, &xe_emlrtRTEI, true);
  emxInit_creal_T(sp, &T, 2, &xe_emlrtRTEI, true);
  st.site = &ke_emlrtRSI;
  b_schur(&st, A, Q, T);
  n = A->size[0] - 1;
  j = R->size[0] * R->size[1];
  R->size[0] = T->size[0];
  R->size[1] = T->size[1];
  emxEnsureCapacity_creal_T(sp, R, j, &ve_emlrtRTEI);
  ...
  emxInit_creal_T(sp, &y, 2, &ye_emlrtRTEI, true);
  st.site = &pe_emlrtRSI;
...
void sqrtm(const emlrtStack *sp, const emxArray_real_T *A, emxArray_creal_T *X)
{
  ...
  R->size[0] = T->size[0];
  R->size[1] = T->size[1];
  emxEnsureCapacity_creal_T(sp, R, i, &ve_emlrtRTEI);
  loop_ub = T->size[0] * T->size[1];
  for (i = 0; i < loop_ub; i++) {
    R->data[i].re = 0.0;
    R->data[i].im = 0.0;
  ...
  emxFree_creal_T(&T);
  emxInit_real_T(sp, &x, 2, &we_emlrtRTEI, true);
  st.site = &se_emlrtRSI;
  check_for_cancellation(&st, R);
  i = x->size[0] * x->size[1];
  x->size[0] = X->size[0];
  x->size[1] = X->size[1];
  emxEnsureCapacity_real_T(sp, x, i, &we_emlrtRTEI);
