...
emxArray_struct0_T *emxCreateND_struct0_T(int numDimensions, const int *size)
{
  ...
  int i;
  int numEl;
  emxInit_struct0_T1(&emx, numDimensions);
  numEl = 1;
  for (i = 0; i < numDimensions; i++) {
    numEl *= size[i];
    emx->size[i] = size[i];
...
{
  emxInit_struct0_T1(pEmxArray, numDimensions);
}
...
/*
 * File trailer for SKLDBarycenter_emxAPI.c
 *
 * [EOF]
 */
