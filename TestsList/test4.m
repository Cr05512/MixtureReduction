function [gm,newParams] = test4(numComp,r,beta)
% [gm,d,alpha] = test4(numComp,r,beta):
% INPUTS:
% - numComp, number of desired components for the full mixture (scalar),
% - r, distance from the origin of the component means (scalar),
% - beta, covariance tuning parameter (scalar).
% OUTPUTS:
% - gm, a Gaussian mixture.
% - d, new state dimension parameter; this is needed for the main loop computations (scalar),
% - alpha, new alpha (scalar).
...
% Gaussian mixture, but with one component having a larger weight than the
% others.
