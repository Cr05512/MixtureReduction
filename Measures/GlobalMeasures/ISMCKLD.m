function ISMCKLDs = ISMCKLD(gmh,gmrVec,nPoints)
...
% - nPoints, number of i.i.d. samples to evaluate the ISMCKLD (scalar).
% OUTPUTS:
...
if isstruct(gmrVec)
    ...
    ISMCKLDs = 1/nPoints * sum(log(valsh./valsr) + valsr./valsh - 1);
else
    for i=1:numGs
        valsr = evalGM(gmrVec{i},X);
        ISMCKLDs(i) = 1/nPoints * sum(log(valsh./valsr) + valsr./valsh - 1);
    end
end
