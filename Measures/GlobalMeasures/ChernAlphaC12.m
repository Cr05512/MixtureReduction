function dCAC = ChernAlphaC12(gmh,gmr,nPoints,alpha)
...
alphaMu = max(max(abs(errorEll)));
    
if d==1
   
    X = linspace(-alphaMu+offset, alphaMu+offset,nPoints);
    ...
    dCAC = trapz(X,f);
elseif d==2
    x1 = linspace(-alphaMu + offset(1), alphaMu + offset(1),nPoints);
    x2 = linspace(-alphaMu + offset(2), alphaMu + offset(2),nPoints);
