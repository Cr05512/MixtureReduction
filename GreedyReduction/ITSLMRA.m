function [gmr,pairs,minCosts] = ITSLMRA(gmh, Nr, maxiter, tol)
% gmr = ITSLMRA(gmh, Nr):
% INPUTS:
...
%     return
% elseif(Nr==1)
%     gmr = TSLBarycenter(gmh,maxiter,tol);
%     return
% end
...
for i=1:Nh
    for j=1:Nh
        if(i<j)
            BMatrix(i,j) = TSLBij(gmr(i),gmr(j),maxiter,tol);
        end
    end
end
...
for k=1:Nh-Nr
    ...
    bar = TSLBarycenter(gmr([i,j]),maxiter,tol);
%     diff = norm(CTD(gmr,[gmr(setdiff(1:numel(gmr),[i,j]));bar],'TSLij')-BMatrix(i,j))
%     if diff>1e-6
%         diff
%         pause
%     end
    ...
    for j=upd_ind
        newBound = TSLBij(bar,gmr(j),maxiter,tol);
        if i<j
