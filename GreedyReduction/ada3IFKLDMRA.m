function [gmr,pairs,minCosts,Nr] = ada3IFKLDMRA(gmh, alpha, RTL)
% gmr = Runnalls(gmh, Nr):
% INPUTS:
...
pbar = FKLDBarycenter(gmh);
maxCost = evalBarycenterFun(gmh,pbar,'FKLDij');
...
%     alpha*evalBarycenterFun(gmr,pbar,'FKLDij')*2/m
    if minCosts(k) > alpha*evalBarycenterFun(gmr,pbar,'FKLDij')*2/m + 4*eps || normCumCTD > RTL
        Nr = numel(gmr);
        break;
    end
