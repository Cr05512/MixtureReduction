function [gmr,pairs,minCosts,Nr] = adaIH2MRA(gmh, perc)
% gmr = Runnalls(gmh, Nr):
% INPUTS:
...
bar = H2Barycenter_mex(gmh,1000,1e-12);
maxCost = evalBarycenterFun(gmh,bar,'H2ij');
...
for k=2:Nh
    ...
    minCosts(k) = minCosts(k-1) + BMatrix(i,j)/maxCost;
    
    if minCosts(k)>perc
        Nr = numel(gmr);
        break;
    end
    bar = H2Barycenter_mex(gmr([i,j]),1000,1e-12);
    gmr(i) = bar;
    gmr(j) = [];
    pairs(k-1,:) = [i,j];
