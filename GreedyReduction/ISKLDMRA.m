function [gmr,pairs,minCosts] = ISKLDMRA(gmh, Nr, maxiter, tol)
% gmr = Runnalls(gmh, Nr):
% INPUTS:
...
if(Nh==Nr)
    return
elseif(Nr==1)
    gmr = SKLDBarycenter(gmh,maxiter,tol);
    return
end
...
for i=1:Nh
    for j=1:Nh
        if(i<j)
            BMatrix(i,j) = SKLDBij(gmr(i),gmr(j),maxiter,tol);
        end
    end
end
...
for k=1:Nh-Nr
    ...
    for j=upd_ind
        newBound = SKLDBij(bar,gmr(j),maxiter,tol);
        if i<j
