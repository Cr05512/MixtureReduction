function [gmr,pairs,minCosts] = INL2MRA(gmh, Nr, maxiter, tol)
% gmr = INL2MRA(gmh, Nr):
% INPUTS:
...
%     return
% elseif(Nr==1)
%     gmr = NL2Barycenter(gmh,maxiter,tol);
%     return
% end
...
for i=1:Nh
    for j=1:Nh
        if(i<j)
            BMatrix(i,j) = NL2Bij(gmr(i),gmr(j),maxiter,tol);
        end
    end
end
...
for k=1:Nh-Nr
    ...
    bar = NL2Barycenter(gmr([i,j]),maxiter,tol);
%     diff = norm(CTD(gmr,[gmr(setdiff(1:numel(gmr),[i,j]));bar],'NL2ij')-BMatrix(i,j))
%     if diff>1e-6
%         diff
%         pause
%     end
    ...
    for j=upd_ind
        newBound = NL2Bij(bar,gmr(j),maxiter,tol);
        if i<j
