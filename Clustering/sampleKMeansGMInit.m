...
% gm_init = sampleKMeansGMInit(gmh,Nr,nSteps,nSamples):
% INPUT:
% - gmh, a Gaussian Mixture,
% - Nr, number of desired components for the reduced mixture (scalar),
% - nSteps, number of KMeans steps (scalar),
% - nSamples, number of samples (scalar).
% OUTPUT:
% - gm_init, an init Gaussian Mixture.
% This function generates an initial Gaussian mixture with Nr components by
% using the KMeans over nSamples drawn from gmh.
if nargin < 3
    nSteps = 100;
    nSamples = 10000;
elseif nargin < 4
